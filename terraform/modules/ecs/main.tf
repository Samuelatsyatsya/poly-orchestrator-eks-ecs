...
# Determine the active AWS region for logging config.
...
locals {
  ...
  app_image = length(trim(var.app_image)) > 0 ? var.app_image : "${aws_ecr_repository.this.repository_url}:latest"
}
...
# Security group for ECS tasks (only ALB -> task traffic).
...
# IAM role used by ECS tasks for pulling images and logging.
...
# IAM role available to the application containers.
...
# ALB listener forwarding HTTP to the target group.
...
# ECS task definition for the app container.
...
# ECS service to run and maintain tasks.
